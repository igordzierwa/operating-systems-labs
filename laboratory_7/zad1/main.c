...
//**MUTEX oraz ZMIENNE WARUNKOWE**//
pthread_mutex_t mutexBuffer = PTHREAD_MUTEX_INITIALIZER; //makro którym może być inicjalizowany mutex atrybutami domyślnymi.
pthread_cond_t condFullBuffer = PTHREAD_COND_INITIALIZER; //makro którym może być inicjalizowana zmienna warunkowa atrybutami domyślnymi.
pthread_cond_t condEmptyBuffer = PTHREAD_COND_INITIALIZER;
...
void cleaner() {
  ...
  pthread_cond_destroy(&condFullBuffer); //usunięcie zmiennej warunkowej
  pthread_cond_destroy(&condEmptyBuffer);
  pthread_mutex_destroy(&mutexBuffer); //system zwalnia zasoby zajęte przez muteks.
}

void *producentFunction() {
  char* line; //wiersz który jest aktualnie czytany
  size_t size;

  while(1) {
    pthread_mutex_lock(&mutexBuffer); //blokada, którą może uzyskać tylko jeden wątek

    while(bufferCounter == N) { //pełny bufor
      pthread_cond_wait(&condFullBuffer, &mutexBuffer); //zawieszenie bieżącego wątku w kolejce związanej ze zmienną warunkową, jednocześnie blokada mutex zostaje zwolniona
    }

    if (getline(&line, &size, fileToRead) <= 0) { //nic nie przeczytano, zwalniamy zasób
      pthread_mutex_unlock(&mutexBuffer); //brak wątków na mutexie - staje się wolny, są wątki zablokowane na mutexie - jeden czekający wątek jest odblokowany
      break;
    }
    ...
    if (bufferCounter == 1) { //buffor nie jest pusty
      pthread_cond_broadcast(&condEmptyBuffer); //wszystkie wątki zablokowane na zmiennej warunkowej zostaną zwolnione
    }

    pthread_mutex_unlock(&mutexBuffer); //odblokowanie blokady
  }
}
...
void *consumerFunction() {
  ...
  while (1) {
    ...
    while(bufferCounter == 0) { //pusty buffor
      pthread_cond_wait(&condEmptyBuffer, &mutexBuffer); //zawieszenie bieżącego wątku w kolejce związanej ze zmienną warunkową, jednocześnie blokada mutex zostaje zwolniona
    }
    ...
    if (bufferCounter == N - 1) { //buffer jest pełny
      pthread_cond_broadcast(&condFullBuffer); //wszystkie wątki zablokowane na zmiennej warunkowej zostaną zwolnione
    }
